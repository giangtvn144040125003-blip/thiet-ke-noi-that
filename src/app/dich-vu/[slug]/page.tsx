...
export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const service = result.data;
  if (!service) notFound();
  return <><SiteHeader /><main className="detail-page"><p className="eyebrow">Dịch vụ</p><h1>{service.name}</h1><p className="page-lede">{service.summary}</p><div className={imageStyles.image}><Image src={service.coverImage ?? "/images/service-design-hd-v3.webp"} alt={`Hình ảnh minh họa ${service.name}`} fill quality={90} sizes="(max-width: 900px) 100vw, 900px" /></div><section><h2>Lợi ích chính</h2><ul>{service.benefits.map((benefit) => <li key={benefit}>{benefit}</li>)}</ul></section><section><h2>Hạng mục bàn giao</h2><ul>{service.deliverables.map((item) => <li key={item}>{item}</li>)}</ul></section><Link className="button" href="/lien-he">Nhận tư vấn cho dự án</Link></main><SiteFooter /></>;
}
